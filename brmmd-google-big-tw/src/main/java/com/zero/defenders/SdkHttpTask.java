...
public class SdkHttpTask extends AsyncTask<String, Void, String> {
    ...
    private String mKeyValueString;
    
    private JSONObject mKeyValueJSONObjel;

    private boolean mIsHttpPost;
    
    private int PostRequst;
    ...
    public void doPost(SdkHttpListener listener,
        ...
        this.mListener = listener;
        this.mIsHttpPost = true;
        this.mKeyValueString = keyValueArray;
        this.mRetryCount = 0;
        this.PostRequst = 1;
        execute(url);
    }
    ...
    public void doPost(SdkHttpListener listener,
            JSONObject keyValueArray,
            String url) {
        this.mListener = listener;
        this.mIsHttpPost = true;
        this.mKeyValueJSONObjel = keyValueArray;
        this.mRetryCount = 0;
        this.PostRequst = 2;
        execute(url);
    }
...
//            Log.d(TAG, this.toString() + "||onResponse");
    ...
    private HttpResponse executeHttp(Context context, String uri) throws SSLHandshakeException,
    	...
    	if(mIsHttpPost)
    	{
    		...
    		{
    			PostRequst = -1;
    			return post(context, uri, mKeyValueArray);
    		}
    		else if(PostRequst == 1)
    		{
    			PostRequst = -1;
    			return post(context, uri, mKeyValueString);
    		}
    		else if(PostRequst == 2)
    		{
    			PostRequst = -1;
    			return post(context, uri, mKeyValueJSONObjel.toString());
    		}
    	}
    	else
    	{
    ...
    private static HttpResponse post(Context context, String url, String params)
            throws SSLHandshakeException, ClientProtocolException, IOException {
        boolean isParam = params != null;
        url = url.replaceAll(" ", "%20");
        HttpClient httpClient = new DefaultHttpClient();
