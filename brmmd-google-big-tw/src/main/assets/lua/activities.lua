...
local Language = luanet.import_type("Language")
local LanguageManager = luanet.import_type("LanguageManager")
local GTSceneManager = luanet.import_type("GTSceneManager")
local PRIVATESERVER = luanet.import_type("PRIVATESERVER")
...
return {
	...
	Update = function(self)
		if (0 ==self.game_mark1) then
		
		else
			local time1 = os.time()			
			if 3 <= (time1 - self.game_time1) then
				self.game_mark1 = 0
				self:OnFinish1()				
			end
		end
	...
	--显示获取到的物品
	OnFinish1 = function(self)
		local id = {}
		local count = {}
		table.insert(id,self.game_diamond_id)
		table.insert(count,self.game_diamond_num)

		ItemDialog.GetInstance():openRecvDialogForLua(self.window.scene,#(id),id,count)
		self.rotate_click = true
	end,
	...
	RecvActiLogin= function(self, rr)
		...
		for i = 1, num do
			if 	1 == list.activitylist[i].isactivity then 
				...
				infos = infos.."s"
				if infos == "1009s" then 
				if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
						infos = infos.."_english"
				else
				...
				FunctionClass.ChangeTextureForLua(clone:Find("off/icon"),"activity/"..infos..".png")
				--GTDebug.LogError("activity/"..infos);
				...
				if list.activitylist[i].buttontype ==1 then
					clone:Find("daily").gameobject:SetActive(true)
				else
					clone:Find("daily").gameobject:SetActive(false)
				end
				if list.activitylist[i].buttontype == 2 then
					clone:Find("timelimit").gameobject:SetActive(true)
				else
					clone:Find("timelimit").gameobject:SetActive(false)
				end
			end
		end		
		...
		self.cur_item = 1
		self.cur_click = list.activitylist[self.cur_item].activityid
		--self.m_model:UpdateModel("SendDetailRequest",jsonstr)
	...
	LoadScene = function(self, scene_id)
		if  self.window.transform:Find(tostring(scene_id)) == nil then 
			...
			--(self.refresh_list[tonumber(self.cur_item)] == false or)
			if  self.refresh_list[tonumber(self.cur_item)] == false or lastsub ~= self.activitysubid[tonumber(self.cur_item)] or self.activityid[tonumber(self.cur_item)]==1014 then 
	...
	RecvDetailRequest= function(self, rr)
		...
		if 1001==actinfo.activityid then
			...
			--逍遥和185渠道时VIP7购买基金			
				GTTools.ChangeLabeText(_path:Find("title"), "t2", "VIP7")	
				GTTools.ChangeLabeText(_path:Find("title/banner"), "n1", "1000")
				GTTools.ChangeLabeText(_path:Find("title/banner"), "n2", "30000")
			...
			for i=1,num do
				...
				GTTools.GetTransformComponent(_path:Find("title/n1"), "LabelSetting").key="30"
			...
			if self.cur_click == actinfo.activityid then
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then		
					--GTTools.GetTransformComponent(_path:Find("title/banner/n1"), "UILabel").fontSize=25			
					_path:Find("title/banner/n1").transform.localPosition=Vector3(-140,0,0)
					
					_path:Find("title/banner/t2").transform.localPosition=Vector3(20,0,0)
					
					--GTTools.GetTransformComponent(_path:Find("title/banner/n2"), "UILabel").fontSize=30		
				    --_path:Find("title/banner/n2").transform.localPosition=Vector3(185,0,0)
					
					GTTools.GetTransformComponent(_path:Find("title/t1"), "UILabel").fontSize=25			
					_path:Find("title/t1").transform.localPosition=Vector3(-100,-32,0)
					
					GTTools.GetTransformComponent(_path:Find("title/n1"), "UILabel").fontSize=35			
					_path:Find("title/n1").transform.localPosition=Vector3(-160,-32,0)
							
					_path:Find("title/Sprite").transform.localPosition=Vector3(90,-32,0)
					
					_path:Find("title/banner/diamond1").transform.localPosition=Vector3(-75,0,0)
					
					--_path:Find("title/banner/diamond2").transform.localPosition=Vector3(248,0,0)
					
					GTTools.GetTransformComponent(_path:Find("title/t2"), "UILabel").text="VIP2+"
					
					_path:Find("title/t2").transform.localPosition=Vector3(-180,-72,0)
					
					_path:Find("title/t3").transform.localPosition=Vector3(-60,-72,0)
        
					_path:Find("title/cur_vip").transform.localPosition=Vector3(60,-72,0)						
		...
		elseif 1002 == actinfo.activityid then 
			...
			if obj ~= nil then
				...
				GTDebug.LogError(_path+"/_path is null!")
			end
			self.can_click = false	
		--钻石消费
		...
		elseif 1004 == actinfo.activityid then 
			...
			local str = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1004 time1")
		...
		elseif 1009 == actinfo.activityid then 
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
					_path:Find("title/label2").gameObject:SetActive(false)
			else
					_path:Find("title/label2").gameObject:SetActive(true)
				end
			self.rewardlist[t] = actinfo.bonusinfo.rewardlist
			local title = actinfo.name
			GTTools.ChangeLabeText(_path:Find("title"), "name", title)	
			if 0== openstatus then 
				local str = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1009 time1")
				GTTools.ChangeLabeText(_path:Find("title"), "info", str)
				--_path:Find("title/info").gameObject:SetActive(false) 
		...
		elseif 1011 == actinfo.activityid then
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
				local info1=_path:Find("title/info1")
				local name=_path:Find("title/name") 
				local title1=_path:Find("title/1")
				local title2=_path:Find("title/2")
				local title3=_path:Find("title/3")
				local title4=_path:Find("title/4")
				
				
				name.localPosition=Vector3(-107,46,0);
				
				GTTools.GetTransformComponent(info1, "UILabel").fontSize=16
				info1.localPosition=Vector3(-255,-105,0);
				
				GTTools.GetTransformComponent(title1, "UILabel").fontSize=18
				title1.localPosition=Vector3(-176,-105,0)
				
				GTTools.GetTransformComponent(title2, "UILabel").fontSize=18
				title2.localPosition=Vector3(-31.5,-105,0)
			
				GTTools.GetTransformComponent(title3, "UILabel").fontSize=18
				title3.localPosition=Vector3(76,-105,0)
				
				GTTools.GetTransformComponent(title4, "UILabel").fontSize=18
				title4.localPosition=Vector3(192,-105,0)
				
				_path:Find("title/starpanel/star").localPosition=Vector3(55,45,0)
				
							
			else
				str = tostring(discount)..Localization.Get("activities 1014 discount")
			end
			...
			GTTools.GetTransformComponent(_path:Find("title/starpanel/"), "UIPanel").depth = self.panel_depth + 6;	
			...
			for i = 1,num,1 do 
				...
				for j = 1,n do
					...
					if self.rewardlist[t][i].rewarditem[j].itemid == 1440600000 and self.rewardlist[t][i].isdouble == 1 then							
						local count3 = self:DoubleToInt(tonumber(self.rewardlist[t][i].rewarditem[j].count)/2)
						self:Item_BG(tempt,tonumber(count3),item_id)
						tempt.transform:Find("double").gameObject:SetActive(true)						
					else						
						local count2 = self.rewardlist[t][i].rewarditem[j].count
						self:Item_BG(tempt,count2,item_id)
						tempt.transform:Find("double").gameObject:SetActive(false)
					end
					if self.rewardlist[t][i].rewarditem[j].limit ==1 then
						tempt.transform:Find("limit").gameObject:SetActive(true)
					else
						tempt.transform:Find("limit").gameObject:SetActive(false)
					end
		...
		elseif 1016 == actinfo.activityid then
			...
			local endtime = _path:Find("title/timelabel")
			...
			for i = 1,num,1 do 
				...
				clone:Find("banner/1").localPosition=Vector3(-35,0,0)
				clone:Find("banner/p").localPosition=Vector3(108,0,0)
				
			   if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.Chinese then		
					clone:Find("banner/p").localPosition=Vector3(70,0,0)
					--GTTools.GetTransformComponent(clone:Find("banner/1"), "LabelSetting").useLocalization=false
					--GTTools.ChangeLabeText(clone, "banner/1", "activities 1016 charge")				

					else
					
					GTTools.GetTransformComponent(clone:Find("banner/1"), "UILabel").fontSize=16
					GTTools.GetTransformComponent(clone:Find("banner/1"), "LabelSetting").useLocalization=false
					GTTools.ChangeLabeText(clone, "banner/1", "[ffedcf]Single Top-up:              USD")
					clone:Find("banner/1").localPosition=Vector3(-30,0,0)
					clone:Find("banner/p").localPosition=Vector3(100,0,0)
					GTTools.GetTransformComponent(clone:Find("banner/info"), "LabelSetting").useLocalization=false
					GTTools.ChangeLabeText(clone, "banner/info", "[ffedcf]Left: [ffea32]   / 1")	
					clone:Find("banner/info").localPosition=Vector3(405,0,0)
					clone:Find("banner/remain").localPosition=Vector3(412,0,0)
				end	
				...
				local costStr = GTTools.formatString(Localization.Get("activities 1016 charge"),cost_num)
				GTTools.ChangeLabeText(clone, "banner/1", costStr)
		...
		elseif 1017 == actinfo.activityid then
			...
			local str = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1017 time1")
			...
			for i = 1,num,1 do 
				...
				local showStr = GTTools.formatString(Localization.Get("activities 1017 keeplogin"),day)
				GTTools.ChangeLabeText(clone, "banner/1", showStr)
			...
			if Language.nation == Language.Nation.Thailand then
				local v = _path:Find("title/timelabel").localPosition
				v.x = -148
				_path:Find("title/timelabel").localPosition = v
			end
		...
		elseif 1018 == actinfo.activityid then
			...
			--local str = self:ShowEndTime1(actinfo.timelist[1].endtime,"activities 1016 time")
			local str = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1018 time1")
			GTTools.ChangeLabeText(_path:Find("title"), "timelabel", str)
			...
			for i = 1,num,1 do 
				...
				local str = GTTools.formatString(Localization.Get("activities 1018 keeplogin"),cost_num)
				GTTools.ChangeLabeText(clone, "banner/1", str)
			   ...
			   if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then		
								clone:Find("banner/1").localPosition=Vector3(-30,0,0)	
								GTTools.GetTransformComponent(clone:Find("banner/1"), "UILabel").fontSize=16
								clone:Find("banner/p").localPosition=Vector3(143,0,0)	
								clone:Find("banner/chargemax").localPosition=Vector3(445,-3,0)	
					else
					
					--GTTools.GetTransformComponent(clone:Find("banner/1"), "LabelSetting").useLocalization=false
					--GTTools.ChangeLabeText(clone, "banner/1", "activities 1018 keeplogin")	
		...
		elseif 1019 == actinfo.activityid then
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
					_path:Find("rank").localPosition=Vector3(440,71,0)
			else
					str = tostring(discount)..Localization.Get("activities 1014 discount")
			end
			...
			local item_id = 0;
			local item_num = 0;
			local time2 = _path:Find("potencylabel")
			local str = self:ShowEndTime1(actinfo.timelist[1].endtime,"activities 1019 info1")
			local str2 = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1019 time1")
			GTTools.ChangeLabeText(time1, "", str)
			GTTools.ChangeLabeText(time2, "", str2)
			...
			for i = 1,num,1 do 
				...
				if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
					clone:Find("banner/1").localPosition=Vector3(-25,0,0)
					clone:Find("banner/p").localPosition=Vector3(151,0,0)
				else
					str = tostring(discount)..Localization.Get("activities 1014 discount")
				end
				...
				if rankmax == rankmin then 
					ranStr = tostring(rankmax)
					--GTTools.ChangeLabeText(clone, "banner/p", tostring(rankmax))
				else
					ranStr = tostring(rankmin).."-"..tostring(rankmax)
					--GTTools.ChangeLabeText(clone,"banner/p",tostring(rankmin).."-"..tostring(rankmax))
				end
				ranStr = GTTools.formatString(Localization.Get("activities 1019 info3"),ranStr)
				GTTools.ChangeLabeText(clone,"banner/1",ranStr)
		...
		elseif 1015 == actinfo.activityid then
			...
			for i = 1,num,1 do 
				...
				local str = GTTools.formatString(Localization.Get("activities 1015 rank"), required_rank)
				GTTools.ChangeLabeText(clone, "banner/info", str)
				GTTools.ChangeLabeText(clone, "banner/p", tostring(cost_num))
		...
		elseif 1014 == actinfo.activityid then
			...
			local str2 = self:ShowTime(actinfo.timelist[1].starttime,actinfo.timelist[1].endtime,"activities 1014 time1")
			...
			for i = 1,num,1 do				 
				...
				if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
					str = tostring(100-discount*10)..Localization.Get("activities 1014 discount")
				else
					str = tostring(discount)..Localization.Get("activities 1014 discount")
				end
		   ...
		   if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
						_path:Find("title/name").localPosition=Vector3(-72,55,0)
						_path:Find("title/info1").localPosition=Vector3(-233,0,0)
			else
		...
		elseif 1013 == actinfo.activityid then
			...
			for i=1,num,1 do			
				...
				if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then							
					GTTools.GetTransformComponent(clone:Find("title/Label"), "UILabel").fontsize=16
		...
		elseif 1022 == actinfo.activityid then
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
				...
				GTTools.GetTransformComponent(_path:Find("role/cur"), "UILabel").fontsize=18
			else
				str = t..cur_re..t1	
				GTTools.ChangeLabeText(_path:Find("role"),"cur",str)
			end
			...
			for i = 1,fbcount do
				...
				if ( i < get_pro or actinfo.bonusinfo.rewardlist[1].status == 2) then
			...
			if LanguageManager.GetInstance().Current_Language == LanguageManager.Nation_Language.English then
						GTTools.GetTransformComponent(_path:Find("buytext"), "UILabel").fontsize=16
				else
		...
		elseif 1023 == actinfo.activityid then
			...
			GTTools.GetTransformComponent(_path:Find("dot"), "UIPanel").depth = self.panel_depth + 3;
			GTTools.GetTransformComponent(_path:Find("btn"), "UIPanel").depth = self.panel_depth + 5;
			local title = actinfo.name
			...
			if cur_get >0 then
				...
				local str5 = Localization.Get("activities 1023 finish")
				GTTools.ChangeLabeText(_path:Find("diamond/consume"), "Label",str5)
			end
	...
	RecvActiOperate = function(self, rr)
		...
		elseif 7 == retype then
			if 0 == operateinfo.result then	
				...
				self.game_diamond_id = 1440600000
				self.game_diamond_num = num
				local num_table = {}
				local count = 0
				while (num ~=0) do
				...
				for j = 1,count do
					...
					local pos = Vector3(0, 660*(5+self.game_num)+66*num_table[j], 0)
					SpringPanel.Begin(go,pos,3)
					if(j == count) then	
						self.game_mark1 = 1 
						self.game_time1 = os.time()
					end
				...
				if operateinfo.curcost == 0 then
					local str5 = Localization.Get("activities 1023 finish")
					GTTools.ChangeLabeText(self.window.transform:Find("1023/diamond/consume"), "Label",str5)
				end
			elseif 1 == operateinfo.result then
				self.remind_mark = true
				self.rotate_click = true
	...
	OnUIEvent = function(self, event)		
		...
		elseif send_name == "buy" then		    
			...
			if 1001 == id then 
				...
				if self.playerinfo.viplevel<vipLv then	
					local str = GTTools.formatString(Localization.Get("mall_viponly"),vipLv)

					GTUIManager.OpenDialogForLua(self.window.scene,str, DialogType.OKCancel, "BuyVip1Callback",self)
				else
		...
		elseif send_name == "game" then	
			if self.rotate_click == false then				
				return
			else
				if self.remind_mark == false then
					self.rotate_click = false
				end
        ...
        if send_name ~="close" and send_name~="buy1" and send_name~="buy" and send_name~="get" and send_name~="goto" and send_name ~="exchange"  and send_name ~= "property" and send_name ~= "arrow" and send_name ~= "extract" and send_name ~= "reg" and send_name ~= "game" and (last ~= self.activityid[tonumber(send_name)] or lastsub ~= self.activitysubid[tonumber(send_name)]) then
				self.cur_click = self.activitylist[tonumber(send_name)].activityid
				...
				--	self.changing_activity_tab = true
					...
					if obj ~= nil then					
						...
						GTDebug.LogError(last+"/Last bj id null!")
					end
